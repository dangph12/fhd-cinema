...
//     // Hàm điều hướng đến trang chọn ghế với showtimeId và các thông tin liên quan đến phim
//     const goToSeatSelection = (showtimeId) => {
//         navigate('/seatSelection', { state: { showtimeId, movieDetails, moviePosterUrl } });
//     };
...
//         const getShowTimes = async () => {
//             try {
//                 const res = await fetchShowTime(movieId); // Sử dụng movieId để lấy showtimes
//                 if (res.data && res.data.data) {
...
//         getShowTimes();
//     }, [movieId]); // Chú ý sử dụng movieId để tìm kiếm suất chiếu
...
const FilmTime = ({ movieDetails, moviePosterUrl }) => {
  ...
  const goToSeatSelection = (showtimeId) => {
    navigate("/seatSelection", {
      state: { showtimeId, movieDetails, moviePosterUrl },
    });
  };
